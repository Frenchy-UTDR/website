...
/* ── Game loop control ───────────────────────────────────────────────────────
   Prevents multiple RAF loops from accumulating when switching slots.
   Set to true only inside startGame(), false on menu return or slot switch.  */
...
function gameTick(now) {
  if (!_gameLoopActive) return; // slot switch / menu return — stop this loop
  if (!G.won) {
...
function startGame(slotN) {
  ...
  // ── Stop any previously running game loop ───────────────────────────────
  _gameLoopActive = false;

  // ── Reset G to a clean slate BEFORE loadGame() ─────────────────────────
  // Without this, a new/empty slot inherits the previous slot's G state
  // and autosave immediately overwrites the wrong key with wrong data.
  G = createDefaultState();
  ...
  for (const id in _mHist) delete _mHist[id];
  _lastMinerSec    = 0;
  _selMinerId      = "axc";
  ...
  _secretSpamClicks         = 0;
  _secretSpamWindow         = [];
  _secretBrokeTriggered     = false;
  _secretWitnessedWhisper   = false;
  _secretCascadeBuy         = false;
  _secretPowerPlayTriggered = false;
  _secretNaviFound          = false;
...
document.getElementById("btn-menu").addEventListener("click", () => {
  saveGame(false);
  _gameLoopActive = false; // stop the game loop while in menu
